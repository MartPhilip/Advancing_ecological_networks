...
XGBOOST_binary <- function(data,bootstrap,core_nbr){
set.seed(42)
cores <- core_nbr
...
bootstrap_data <- pbmcapply::pbmclapply(1:bootstrap, function(j) {
  ...
  y_pred_binary <- as.integer(y_pred > 0.5)
  mcc_value <- mltools::mcc(preds = y_pred_binary, actuals = Y_test)
  data_new <- data[-train_indices,]
  species <- data_new |> dplyr::select(scientific_name)
  species <- cbind(species,y_pred) 
  species$boostrap <- j
  species$mcc_value <- mcc_value
